...
public class DroneControllerServerOrig extends Thread {
    ...
    public synchronized void setFlyingMessage(FlyingMessage flyingMessage) {
         ...
         *
         * - 해당 Drone에게 비행 중지 메시지를 전송한다.
         */
        if(flyingMessage == FlyingMessage.STATUS_NEED_STOP_FLYING){
            // TODO 누구로 부터 받은 메시지인지 표시 필요.

            DroneRunner droneRunner = DroneController.droneRunnerRepository.sendMessageToFollower(FlyingMessage.STATUS_NEED_STOP_FLYING,
